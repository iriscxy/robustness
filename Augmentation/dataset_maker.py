...
class DatasetMaker:
    ...
    def make_dataset(self):
        ...
        def preprocess_function(examples: Dict):
            ...
            if isinstance(examples["aug_src"][0], str):
                aug_inputs = [ex.replace(' ', '') if self.data_args.chinese_data else ex for ex in examples["aug_src"]]
            elif isinstance(examples["aug_src"][0], list):
                aug_inputs = [' '.join(ex).replace(' ', '') if self.data_args.chinese_data else ' '.join(ex) for ex in
                          examples["aug_src"]]
            else:
                raise ValueError(f'only support str/list in content, now {type(examples["src"][0])}')
            ...
            aug_model_inputs = self.tokenizer(aug_inputs, max_length=self.data_args.max_source_length, padding=padding,
                                          truncation=True)
            model_inputs['aug_input_ids']=aug_model_inputs['input_ids']
            model_inputs['aug_attention_mask']=aug_model_inputs['attention_mask']
